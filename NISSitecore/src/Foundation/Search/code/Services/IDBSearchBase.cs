...
{
    /// <summary>
    /// Provides base methods for working with Solr IDB Search queries.
    /// </summary>
    public abstract class IDBSearchBase
    {
        /// <summary>
        /// Search for content using an instance of <see cref="IDBSolrQuery"/>.
        /// </summary>
        protected virtual SolrQueryResults<T> IDBSearch<T>(
            string q,
            Item solrQueryItem,
            int startRow = 0,
            string language = "",
            ICollection<ISolrQuery> additionalFilters = null)
        {
            ...
            // Build the IDBSolrQuery object from the Sitecore item.
            IDBSolrQuery query = BuildIDBSolrQuery(solrQueryItem);

            return IDBSearch<T>(q, query, startRow, language, additionalFilters);
        }
        ...
        /// <summary>
        /// Builds the strongly-typed <see cref="IDBSolrQuery"/> instance from the given Sitecore Item id.
        /// </summary>
        protected virtual IDBSolrQuery BuildIDBSolrQuery(Item item)
        {
            if (item == null)
            {
        ...
        /// </summary>
        protected virtual ISearchIndex BuildSearchIndex(IDBSolrQuery query)
        {
        ...
        /// <summary>
        /// Convenience method that gets the Sitecore Field ID for a <see cref="IDBSolrQuery"/> property.
        /// </summary>
        protected virtual Guid FieldIdFor<TOut>(Expression<Func<IDBSolrQuery, TOut>> propertyExpression)
        {
            return GetPropValue<IDBSolrQuery, TOut, SitecoreFieldAttribute, Guid>(propertyExpression, attr => attr.FieldId);
        }
